...
if runner_ok then
    ...
    vim.keymap.set("n", "<leader>tc", function() runner.run() end, { desc = "Ejecutar comando nuevo" })
    
    -- <leader>rl = Run Last (Repite el último comando al instante sin preguntar)
    vim.keymap.set("n", "<leader>tl", function() runner.runLast() end, { desc = "Repetir último comando" })
    
    -- <leader>rt = Run Toggle (Muestra/Oculta la ventana de la terminal sin cerrarla)
    vim.keymap.set("n", "<leader>tt", function() runner.toggle() end, { desc = "Mostrar/Ocultar terminal" })
end
